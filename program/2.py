...
# 删除某些行数据
if black_list:
    df = df[~df['symbol'].isin(black_list)]  # 过滤掉黑名单币种
if white_list:
    df = df[df['symbol'].isin(white_list)]  # 只保留白名单币种
...
# print(df[df['symbol']=='BTC-USDT'].sort_values('candle_begin_time').head(100))
# exit()
# 计算下个周期的收益率
...
# 保留指定字段，减少计算时间
df = df[['candle_begin_time', 'symbol', 'ret_next', '每小时涨跌幅','close','open'] + factor_column_list]
# 删除选币因子为空的数据
...
select_coin = df.copy()
# 保留指定字段
select_coin = select_coin[['candle_begin_time', 'symbol', 'ret_next', '每小时涨跌幅','open','close']]
...
# 计算最终每小时净值变化
index_df['指数净值'] = (index_df['涨跌幅'] + 1).cumprod()
# 构建指数填充字段
index_df['open'] = index_df['指数净值']
index_df['high'] = index_df['指数净值']
index_df['low'] = index_df['指数净值']
index_df['close'] = index_df['指数净值']
index_df['volume'] = 1
# 首行数据单独处理一下
...
# 筛选指定字段保存
col_list = ['candle_begin_time', '选币', 'open', 'high', 'low', 'close', 'volume', '开平仓标识', '指数净值', '涨跌幅']
...
equity['btc_净值'] = (equity['btc_涨跌幅'] + 1).cumprod()
# 增加指数 超额收益 alpha
equity['Alpha'] = (equity['指数净值'] - equity['btc_净值'] ) / equity['btc_净值']
# 增加指数 ROI
equity['ROI'] = (equity['指数净值'] - equity['指数净值'].iloc[0]) / equity['指数净值'].iloc[0]

# 增加指数名字
equity['symbol'] = index_name
col_list = ['candle_begin_time', '选币','开平仓标识', 'open', 'high', 'low', 'close', 'volume', '指数净值', 'btc_净值','Alpha','ROI','symbol']
Q_df = pd.DataFrame()
Q_df= equity[col_list]
# print(Q_df.head(5))
# print(Q_df.tail(5))
# exit()
# 保存文件
...
# ===画图
pic_title = 'factor:%s_nv:%s_pro:%s_risk:%s_selection_num:%s' % (factor_list, rtn.at['累积净值', 0], rtn.at['年化收益', 0], rtn.at['最大回撤', 0],select_num)
draw_equity_curve_mat(equity, data_dict={'指数': '指数净值', 'btc_净值': 'btc_净值'}, date_col='candle_begin_time', title=pic_title)
# 如果上面的函数不能画图，就用下面的画图
