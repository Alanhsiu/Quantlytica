...
def strategy_evaluate(equity):
    ...
    # ===计算累积净值
    results.loc[0, '累积净值'] = round(temp['指数净值'].iloc[-1], 2)

    # ===计算年化收益
    annual_return = (temp['指数净值'].iloc[-1]) ** (
    ...
    # 计算当日之前的资金曲线的最高点
    temp['max2here'] = temp['指数净值'].expanding().max()
    # 计算到历史最高值到当日的跌幅，drowdwon
    temp['dd2here'] = temp['指数净值'] / temp['max2here'] - 1
    # 计算最大回撤，以及最大回撤结束时间
    end_date, max_draw_down = tuple(temp.sort_values(by=['dd2here']).iloc[0][['candle_begin_time', 'dd2here']])
    # 计算最大回撤开始时间
    start_date = temp[temp['candle_begin_time'] <= end_date].sort_values(by='指数净值', ascending=False).iloc[0]['candle_begin_time']
    # 将无关的变量删除
...
def draw_equity_curve_mat(df, data_dict, date_col=None, right_axis=None, pic_size=[16, 9], font_size=25,
                          log=False, chg=False, title=None, y_label='净值', save_title=None):
    """
    绘制策略曲线
    :param df: 包含净值数据的df
